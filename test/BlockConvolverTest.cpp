...
template <typename T>
std::unique_ptr<T[], free_type> generate_random(size_t len, size_t num_nonzero, long int seedval)
{
  ...
  public:
    ...
    void run_test_convolve(const std::string &out_dir="")
    {
      ...
      for (size_t i = 0; i < irs.size(); i++)
      {
        ...
        for (size_t block = 0; block < num_blocks; block++)
        {
          ...
          if      ( last_block &&  this_block)    memcpy(input_for_ir.get() + offset, input.get() + offset, block_size * sizeof(float));
          else if (!last_block &&  this_block)   fade_up(input_for_ir.get() + offset, input.get() + offset, block_size                );
          else if ( last_block && !this_block) fade_down(input_for_ir.get() + offset, input.get() + offset, block_size                );
        }
    ...
    void run_real_convolve(BlockConvolver::Context &ctx)
    {
      ...
      for (size_t block = 0; block < num_blocks; block++)
      {
        ...
        {
          if (this_filter >= 0) convolver->crossfade_filter(&filters[this_filter]);
          else convolver->crossfade_filter(NULL);
        }
        
        // filter the block to the output; possibly passing null if zeros are detected
        if (null_for_zeros && all_zeros(input.get() + offset, block_size))
        {
          convolver->filter_block(NULL, real_output.get() + offset);
        }
        else
        {
          convolver->filter_block(input.get() + offset, real_output.get() + offset);
        }
      }
    }
...
BOOST_AUTO_TEST_CASE( filter_correct_num_blocks )
{
  ...
  auto coeff = generate_random<float>(2000, 100, 0);
